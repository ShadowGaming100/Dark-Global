...
module.exports = async (client) => {
    try {
        ...
        let amount = 0;
        const load_dir = (dir) => {
            const event_files = fs.readdirSync(`${process.cwd()}/Structures/Events/${dir}`).filter((file) => file.endsWith(".js"));
            for (const file of event_files) {
                try {
                    const event = require(`${process.cwd()}/Structures/Events/${dir}/${file}`)
        ...
        await ["client", "guild", "interaction", "message"].forEach(e => load_dir(e));
